...
static Future<bool> addDependency(String dependency) async {
  try {
    ...
    if (!pubspec.existsSync()) {
      print('Error: pubspec.yaml not found in the current project directory: $projectDirectory');
      return false;
    }

    final pubspecContent = await pubspec.readAsString();

    if (pubspecContent.contains(RegExp('^\\s*$dependency:', multiLine: true))) {
    ...
    if (process.exitCode == 0) {
      print('Dependency "$dependency" added successfully to the project at $projectDirectory.');
      return true;
    } else {
